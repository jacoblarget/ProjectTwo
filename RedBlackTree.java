...
// Notes to Grader: found bug in rotate, so I fixed that as well
...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
    ...
    protected static class Node<T> {
        ...
        public String toString() {
            ...
            while(!q.isEmpty()) {
                ...
                // debugging code that checks color
                // if(next.isBlack){
                //     output += " B";
                // } else {
                //     output += " R";
    ...
    /**
     * Recursive function that rotates and recolors as necessary to self-balance this tree.
     * Checks for Red/Black Tree violations after insertion
     * @param newNode is the child where there might be a problem
     */
    private void enforceRBTreePropertiesAfterInsert(Node<T> newNode){
        // this if statement stops recursion once there is no longer a violation
        // at the root, a root child, or no red parent of newNode
        if(newNode.parent != null && newNode.parent.parent != null && !(newNode.parent.isBlack)){
            // here, we have a red child with a red parent, so we need to fix the violation
            if(newNode.parent.isLeftChild()){
                // here, left parent
                // null children are treated black
                if(newNode.parent.parent.rightChild == null || newNode.parent.parent.rightChild.isBlack) {
                    // here, pibling black
                    if(newNode.isLeftChild()){
                        // here, left child
                        // solution is to recolor parent and grandparent, rotate them, and try again
                        ...
                        // here, right child
                        // solution is to rotate 
                        rotate(newNode, newNode.parent);
                        // this was the parent pre-rotate, but now the bottom-most red node in question
                        enforceRBTreePropertiesAfterInsert(newNode.leftChild);
                    }
                } else {
                    // here, pibling red
                    // solution is to recolor and try at next red node (the grandparent)
                    newNode.parent.isBlack = true;
                ...
                if(newNode.parent.parent.leftChild == null || !(newNode.parent.parent.leftChild.isBlack)) {
                    // here, pibling red
                    // solution is to recolor and work up the tree
                    newNode.parent.isBlack = true;
                    newNode.parent.parent.isBlack = false;
                    ...
                    if(newNode.isLeftChild()){
                        // here, left child
                        // solution is to recolor and parent and grandparent, rotate them, and try again
                        newNode.parent.isBlack = true;
                        newNode.parent.parent.isBlack = false;
                        rotate(newNode.parent, newNode.parent.parent);
                        enforceRBTreePropertiesAfterInsert(newNode);
                        ...
                        // solution is to rotate
                        rotate(newNode, newNode.parent);
                        // this was the parent pre-rotate, but now bottom-most red node in question
                        enforceRBTreePropertiesAfterInsert(newNode.leftChild);
                    }
                }
            }
